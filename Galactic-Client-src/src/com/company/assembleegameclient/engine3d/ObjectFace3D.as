...
public class ObjectFace3D
{
    ...
    public function draw(graphicsData:Vector.<IGraphicsData>, color:uint, texture:BitmapData) : void
    {
        ...
        if(!Parameters.data_.HWAcceleration && (!this.useTexture_ || texture == null))
        {
        ...
        {
            if(texture == null && Parameters.data_.HWAcceleration)
            {
                texture = blackBitmap;
            }
            else
        ...
        if(Parameters.data_.HWAcceleration && this.bitmapFill_ != null)
        {
