...
namespace wServer.realm.worlds.logic
{
    public class MoonLord
    {
        ...
        private void BroadcastMsg(string message, int whichone = 1)
        {
            _world.Manager.Chat.Oryx(_world, message, whichone);
        }
        public void InitCloseRealm()
        {
            Closing = true;
            _world.Manager.Chat.Announce("The Moon seems to be shaking... (30 seconds)", true);
            _world.Timers.Add(new WorldTimer(30000, (w, t) => CloseRealm()));
        }

        private void CloseRealm()
        {
            _world.Closed = true;
            _world.Timers.Add(new WorldTimer(10000, (w, t) => SendToCastle()));
        }

        private void SendToCastle()
        {
            BroadcastMsg("YOU WRECKLESS MORTALS, YOU DARE ABUSE MY PERMISSION AND ATTACK MY MINIONS.", 2);
            BroadcastMsg("I WILL VANQUISH YOU WITHIN MINUTES OF YOUR ARRIVAL!",2);
            BroadcastMsg("PREPARE FOR YOUR END!", 2);

            _world.Timers.Add(new WorldTimer(10000, (w, t) => _world.Closed = false));

            if (_world.Players.Count <= 0)
                return;

            var world = _world.Manager.AddWorld(
                new MoonCastle(_world.Manager.Resources.Worlds.Data["Castle of Cyberious"], playersEntering: _world.Players.Count));
